...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [keywordPerf, asinPerf, pastRecs] = await Promise.all([
      loadKeywordPerformance(supabase, bookId, user.id),
      loadCompetitorAsinPerformance(supabase, bookId, user.id),
      loadPastRecommendations(supabase, bookId, user.id),
    ]);

    const generated: Array<Recommendation & { entityKind: "keyword" | "asin" }> = [
      ...keywordPerf.flatMap((kw) => recommendForKeyword(kw, targetAcos).map((r) => ({ ...r, entityKind: "keyword" as const }))),
      ...asinPerf.flatMap((asin) => recommendForCompetitorAsin(asin, targetAcos).map((r) => ({ ...r, entityKind: "asin" as const }))),
    ];
    ...
    if (keywordRows.length > 0) {
      const { error } = await supabase.from("keyword_recommendations").upsert(keywordRows, { onConflict: "keyword_id,type" });
...
async function loadKeywordPerformance(supabase: SupabaseClient, bookId: string, userId: string): Promise<KeywordPerformance[]> {
  ...
  const [{ data: rollups }, campaignTypeByKeyword] = await Promise.all([
    ...
    loadCampaignTypeByKeyword(supabase, bookId, userId, ids),
  ]);
  const rollupById = new Map(((rollups ?? []) as RollupRow[]).map((r) => [r.keyword_id as string, r]));
...
async function loadCompetitorAsinPerformance(
  ...
  const { data: rollups } = await supabase.from("competitor_asin_result_rollups").select("*").in("competitor_asin_id", ids);
  const rollupById = new Map(((rollups ?? []) as RollupRow[]).map((r) => [r.competitor_asin_id as string, r]));
...
async function loadCampaignTypeByKeyword(
  ...
  const { data } = await supabase
    ...
    .select("keyword_id, campaigns!inner(campaign_type, book_id, user_id)")
  ...
  for (const row of data ?? []) {
    ...
    if (row.keyword_id && campaign?.campaign_type) {
      map.set(row.keyword_id as string, campaign.campaign_type as KeywordPerformance["campaignType"]);
    }
  }
  return map;
}
...
async function loadPastRecommendations(
  ...
  for (const row of data ?? []) {
    ...
    list.push({ type: row.type as PastRecommendation["type"], status: row.status as PastRecommendation["status"], decidedAt: row.decided_at as string | null });
