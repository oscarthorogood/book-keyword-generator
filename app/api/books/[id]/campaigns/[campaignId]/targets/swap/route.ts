 ...
 * button will appear").
 */
export async function POST(request: Request, { params }: { params: Promise<{ id: string; campaignId: string }> }) {
  try {
    ...
    if (campaign.status !== "exported") {
      return Response.json({ error: "This campaign hasn't been exported yet — use Create Campaign first." }, { status: 400 });
    }

    const campaignType = campaign.campaign_type as CampaignType;
    const adGroup = SINGLE_AD_GROUP_LABEL[campaignType];
    if (!adGroup) {
      return Response.json({ error: "Manual update isn't supported for Auto Discovery." }, { status: 400 });
    }
    ...
    if (body.createKeyword && typeof body.createKeyword.text === "string" && body.createKeyword.text.trim()) {
      const text = body.createKeyword.text.trim();
      const matchType: MatchType = ["broad", "phrase", "exact"].includes(body.createKeyword.matchType)
        ? body.createKeyword.matchType
        : "phrase";
      const bid = typeof body.createKeyword.bid === "number" && body.createKeyword.bid > 0 ? body.createKeyword.bid : 0.5;
      ...
      if (existing) {
        await supabase.from("keywords").update({ status: "active" }).eq("id", existing.id).eq("user_id", user.id);
        newEntity = { keywordId: existing.id, text: existing.text, matchType: existing.match_type as MatchType, bid: existing.bid };
      } else {
        ...
        newEntity = { keywordId: created.id, text: created.text, matchType: created.match_type as MatchType, bid: created.bid };
      }
    } else if (body.createAsin && typeof body.createAsin.competitorAsin === "string" && body.createAsin.competitorAsin.trim()) {
      const competitorAsin = body.createAsin.competitorAsin.trim().toUpperCase();
      const bid = typeof body.createAsin.bid === "number" && body.createAsin.bid > 0 ? body.createAsin.bid : 0.5;
      ...
      if (existing) {
        await supabase.from("competitor_asins").update({ status: "active" }).eq("id", existing.id).eq("user_id", user.id);
        newEntity = {
        ...
        const { data: created, error: createError } = await supabase
          .from("competitor_asins")
          .insert({ book_id: bookId, user_id: user.id, competitor_asin: competitorAsin, source: "manual", bid })
          .select("id, competitor_asin, bid")
          .single();
        if (createError) return Response.json({ error: createError.message }, { status: 400 });
        newEntity = {
      ...
      if (!row) return Response.json({ error: "Keyword not found in this book's bank." }, { status: 404 });
      newEntity = { keywordId: row.id, text: row.text, matchType: row.match_type as MatchType, bid: row.bid };
      ...
      const { data: row, error } = await supabase
        ...
        .maybeSingle();
      if (error) return Response.json({ error: error.message }, { status: 400 });
      if (!row) return Response.json({ error: "ASIN not found in this book's bank." }, { status: 404 });
      newEntity = {
      ...
      return Response.json({ error: "Missing a replacement keyword, ASIN, or something to create." }, { status: 400 });
    }
